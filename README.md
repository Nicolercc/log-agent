...
Roughly 550 lines of Python. No server, no web UI, no hosted anything. The
...
Phase 1 has zero dependencies. Extras arrive with the later phases:

```bash
pipx install -e ".[gmail]"      # Phase 2
pipx install -e ".[gmail,classify]"   # Phase 3
```
...
thirty emails once, then you can state:
...
40 tests. Every staleness assertion passes an explicit `today` — a suite whose
...
**Hard stop rule:** 1,200 lines of Python excluding tests, across the whole
project. Phase 1 is ~670 (`jt.py` + `transitions.py`), which leaves real room
for sync, classify, and priority without leaving room for a rewrite. If you
pass it, you have started a startup instead of a job search.

An earlier draft of this file set the limit at 600 and `jt.py` alone came in at
617 — a budget you breach on day one is not a budget.
